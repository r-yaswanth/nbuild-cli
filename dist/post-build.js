...
import { EXPORT_OPTIONS_PLIST, androidApkPath, androidDsymsDir, androidFirebaseAppId, buildArchiveDir, exportOptionsPlistPath, googleServiceInfoPlist, iosArchivePath, iosDsymsPath, iosFirebaseAppId, iosIpaDir, iosIpaPath, outputDir, podsDir, } from "./config.js";
import { archiveBuild } from "./archive.js";
import { uploadToGcs } from "./gcs.js";
import { distributeToFirebase, extractProjectNumber, FirebaseReauthRequiredError, getAccessToken, listGroups, listTesters, } from "./distribute.js";
...
async function promptForDistribution(config) {
    ...
    // Release notes
    const userNotes = await new Promise((resolve) => {
        const lines = [];
    ...
    const metaLines = [];
    if (config.gitBranch)
        metaLines.push(`branch: ${config.gitBranch}`);
    if (config.commitId)
        metaLines.push(`commit: ${config.commitId}`);
    if (config.flavor)
        metaLines.push(`environment: ${config.flavor}`);
    ...
    while (!fetchDone) {
        try {
            ...
            if (!appId)
                throw new Error("Could not read Firebase App ID from android/app/google-services.json");
            ...
            // Select groups
            if (groups.length > 0) {
                const selectedGroups = await searchMultiselect("👥 Select tester groups to distribute to", groups.map((g) => ({
                    value: g.name,
                    label: g.displayName,
            ...
            // Select testers
            if (testers.length > 0) {
                const selectedTesters = await searchMultiselect("👤 Select individual testers to distribute to", testers.map((t) => ({
                    value: t.email,
...
// ── Android Post-Build ─────────────────────────────────────────────
async function postBuildAndroid(config) {
    ...
    if (!config.distributeToFirebase) {
        ...
        if (fs.existsSync(dsyms)) {
            const tarResult = await execShell(`tar -czf "${dsymsArchive}" -C "${dsyms}" .`, { cwd: projectRoot });
            if (tarResult.exitCode === 0) {
                p.log.success(`📦 Archived dSYMs to ${pc.green("output/android_dsyms.tar.gz")}`);
            }
            else {
    ...
    if (s)
        s.start("☁️  Uploading Android symbols to Firebase Crashlytics");
    else
        p.log.step(pc.blue("☁️  Uploading Android symbols to Firebase Crashlytics"));
    const uploadResult = await exec("firebase", [
    ...
    if (uploadResult.exitCode === 0) {
        if (s)
            s.stop(pc.green("✅ ☁️  Android symbols uploaded to Firebase Crashlytics"));
        else
            p.log.success("☁️  Android symbols uploaded to Firebase Crashlytics");
    }
    else {
        if (s)
            s.stop(pc.red("❌ Failed to upload Android symbols"));
        else
            p.log.error("❌ Failed to upload Android symbols");
        if (!verbose) {
    ...
    await distributeToFirebase(apk, appId, config, "Android APK");
}
// ── iOS Post-Build ─────────────────────────────────────────────────
async function postBuildIos(config) {
    const { projectRoot } = config;
    const out = outputDir(projectRoot);
    const verbose = isVerbose();
    const s = verbose ? null : clockSpinner();
    ...
    if (s)
        s.start("📤 Exporting iOS IPA (ad-hoc)");
    else
        p.log.step(pc.blue("📤 Exporting iOS IPA (ad-hoc)"));
    const exportResult = await exec("xcodebuild", [
        "-exportArchive",
        "-archivePath",
    ...
    if (exportResult.exitCode !== 0) {
        if (s)
            s.stop(pc.red("❌ iOS IPA export failed"));
        else
            p.log.error("❌ iOS IPA export failed");
        if (!verbose) {
    ...
    if (s)
        s.stop(pc.green("✅ 📤 iOS IPA export successful"));
    else
        p.log.success("📤 iOS IPA export successful");
    if (!config.distributeToFirebase) {
    ...
    if (!iosAppId) {
        ...
        if (s)
            s.start("☁️  Uploading iOS symbols to Firebase Crashlytics");
        else
            p.log.step(pc.blue("☁️  Uploading iOS symbols to Firebase Crashlytics"));
        const dsyms = iosDsymsPath(projectRoot);
        const gspPlist = googleServiceInfoPlist(projectRoot);
        const uploadSymbolsBin = path.join(podsDir(projectRoot), "FirebaseCrashlytics/upload-symbols");
        let uploaded = false;
        // Try upload-symbols binary first
        if (fs.existsSync(uploadSymbolsBin)) {
            const result = await exec(uploadSymbolsBin, ["-gsp", gspPlist, "-p", "ios", dsyms], { cwd: projectRoot });
            if (result.exitCode === 0) {
                if (s)
                    s.stop(pc.green("✅ ☁️  iOS symbols uploaded via upload-symbols"));
                else
                    p.log.success("☁️  iOS symbols uploaded via upload-symbols");
                uploaded = true;
            }
            else {
                if (s)
                    s.message("⚠️  upload-symbols failed, trying firebase CLI...");
                else
                    p.log.warn("⚠️  upload-symbols failed, trying firebase CLI...");
            }
        }
        // Fallback to firebase CLI
        if (!uploaded) {
            const fbResult = await exec("firebase", ["crashlytics:symbols:upload", `--app=${iosAppId}`, dsyms], { cwd: projectRoot });
            if (fbResult.exitCode === 0) {
                if (s)
                    s.stop(pc.green("✅ ☁️  iOS symbols uploaded via firebase CLI"));
                else
                    p.log.success("☁️  iOS symbols uploaded via firebase CLI");
            }
            else {
                if (s)
                    s.stop(pc.red("❌ Failed to upload iOS symbols"));
                else
                    p.log.error("❌ Failed to upload iOS symbols");
                if (!verbose) {
        ...
        await distributeToFirebase(iosIpaPath(projectRoot), iosAppId, config, "iOS IPA");
    }
