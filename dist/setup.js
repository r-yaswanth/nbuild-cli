...
async function checkCli(command, args, cwd) {
...
export async function checkFlutterCli(cwd) {
    return checkCli("flutter", ["--version"], cwd);
}
export async function checkFirebaseCli(cwd) {
...
// ── Firebase Login ────────────────────────────────────────────────
export async function runFirebaseLogin() {
    return new Promise((resolve) => {
        ...
        const printUrl = (data) => {
            ...
            if (match)
                p.log.info(`🌐 Open this URL to log in:\n  ${match[0]}`);
        };
...
// ── Firebase Logout ──────────────────────────────────────────
export async function runFirebaseLogout() {
    return new Promise((resolve) => {
...
async function installFirebaseCli() {
    ...
    const result = await exec("npm", ["install", "-g", "firebase-tools"], { cwd: os.homedir() });
...
export async function setupFirebase(cwd) {
    // Check if Firebase CLI is installed
    const firebase = await checkFirebaseCli(cwd);
    if (!firebase.installed) {
        p.log.warn("⚠️  Firebase CLI is not installed.");
        p.log.info(pc.dim("Firebase CLI is required for Crashlytics symbol upload & App Distribution."));
        const install = await p.confirm({
            message: "📦 Install Firebase CLI now?",
            initialValue: true,
        });
        if (p.isCancel(install) || !install) {
            p.log.warn(`⚠️  Firebase features will be unavailable. Run ${pc.cyan("nlearn-build --firebase-setup")} later to install.`);
            return false;
        }
        const installed = await installFirebaseCli();
        if (!installed) {
            p.log.warn(`⚠️  Installation failed. Try manually: ${pc.cyan("npm install -g firebase-tools")}`);
            return false;
        }
    }
    else {
    ...
    if (!auth.authenticated) {
        ...
        if (p.isCancel(login) || !login) {
            p.log.warn(`⚠️  Firebase features will be unavailable. Run ${pc.cyan("nlearn-build --firebase-setup")} later to login.`);
            return false;
        }
    ...
    // Validate org domain
    if (!auth.isOrgDomain) {
        p.log.warn(`⚠️  Logged in as ${pc.cyan(auth.email ?? "unknown")} — expected @${pc.green(REQUIRED_DOMAIN)} domain.`);
        const relogin = await p.confirm({
            message: `🔑 Re-login with your @${REQUIRED_DOMAIN} account?`,
            initialValue: true,
        });
        if (p.isCancel(relogin) || !relogin) {
            p.log.warn("⚠️  Distribution may fail without the correct org account.");
        ...
        if (auth.isOrgDomain) {
            ...
            p.log.warn(`⚠️  Still logged in as ${pc.cyan(auth.email ?? "unknown")}. Distribution may fail.`);
        }
        return true;
    }
...
export async function runInitialSetup(projectRoot) {
    ...
    if (!flutter.installed) {
        ...
        p.log.info(`Install Flutter: ${pc.cyan("https://docs.flutter.dev/get-started/install")}`);
...
export async function runFirebaseSetupCommand() {
