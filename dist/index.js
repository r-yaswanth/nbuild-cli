...
function parseFlutterExtraArgs() {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (a === "--flutter-args") {
            const raw = argv[i + 1];
            if (raw)
                result.push(...raw.split(/\s+/).filter(Boolean));
            i++;
            continue;
        }
...
async function main() {
    ...
    const config = await gatherBuildConfig(projectRoot, firebaseReady, obfuscate, archivesPath, flutterExtraArgs);
    await runBuildPipeline(config);
    await runPostBuild(config);
    const flavorCapitalized = config.flavor.charAt(0).toUpperCase() + config.flavor.slice(1);
    p.outro(pc.green(`🎉 ${flavorCapitalized} build completed successfully!`));
}
main().catch((err) => {
