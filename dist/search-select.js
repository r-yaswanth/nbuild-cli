 ...
 */
export function searchMultiselect(message, allOptions) {
    return new Promise((resolve) => {
        ...
        function render() {
            ...
            lines.push(`${S_BAR}  ${pc.dim(`${matchCount} match${matchCount !== 1 ? "es" : ""}`)}`
                + `  ·  `
                + (selCount > 0 ? pc.green(`${selCount} selected`) : pc.dim("none selected")));
            lines.push(`${S_BAR_END}  ${pc.dim("[↑↓] navigate   [space] toggle   [enter] confirm   [esc] cancel")}`);
 ...
 */
export function searchSelect(message, allOptions) {
    return new Promise((resolve) => {
        let query = "";
        let cursorIdx = 0;
        let scrollOff = 0;
        ...
        function render() {
            ...
            lines.push(`${S_BAR}  ${pc.dim(`${matchCount} match${matchCount !== 1 ? "es" : ""}`)}`);
            lines.push(`${S_BAR_END}  ${pc.dim("[↑↓] navigate   [enter] select   [esc] cancel")}`);
            process.stdout.write(lines.join("\n") + "\n");
            prevLines = lines.length;
        }
        function finish(cancelled) {
