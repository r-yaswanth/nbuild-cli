...
const ANDROID_TABLE = "`nlearnv4.firebase_crashlytics.com_narayanagroup_nlearnapp_ANDROID_REALTIME`";
const IOS_TABLE = "`nlearnv4.firebase_crashlytics.com_narayanagroup_nlearnapp_IOS_REALTIME`";
...
function logCurlStart(sql, token) {
    if (!isVerbose())
        return;
    const redacted = redactToken(token);
    const sqlOneLine = sql.replace(/\s+/g, " ").trim();
    const sqlShort = sqlOneLine.length > 900 ? sqlOneLine.slice(0, 900) + "…" : sqlOneLine;
...
function flattenRow(row, fields) {
    const result = {};
...
async function runBigQueryQuery(sql, token) {
    ...
    if (!jobId)
        throw new Error("BigQuery: no job ID returned");
    const locationParam = location ? `&location=${encodeURIComponent(location)}` : "";
    // 2. Poll until done (max ~60 seconds)
    const maxAttempts = 30;
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        await sleep(2000);
        const pollResp = await fetch(`${BQ_BASE}/jobs/${jobId}?${locationParam}`, { headers: { Authorization: `Bearer ${token}` } });
        ...
        if (errorResult) {
            throw new Error(`BigQuery job failed: ${errorResult.message ?? "unknown error"}`);
        }
        if (state === "DONE")
            break;
    ...
    // 3. Get results
    const resultsResp = await fetch(`${BQ_BASE}/queries/${jobId}?maxResults=10000&timeoutMs=0${locationParam}`, { headers: { Authorization: `Bearer ${token}` } });
    if (!resultsResp.ok) {
        const body = await resultsResp.text();
        throw new Error(`BigQuery results fetch failed (${resultsResp.status}): ${body}`);
    }
...
export async function fetchCrashesForVersion(platform, displayVersion, buildVersion, token) {
  ...
  `;
    ...
    return rows.map((row) => {
        ...
        const lines = [];
        try {
            const exceptions = JSON.parse(row["exceptions_json"] ?? "[]");
            const ex = exceptions[0];
            if (ex) {
                if (ex.type || ex.exception_message) {
                    lines.push(`${ex.type ?? "Exception"}${ex.exception_message ? ": " + ex.exception_message : ""}`);
                }
...
export async function fetchBuildIdForVersion(platform, displayVersion, buildVersion, token) {
    const table = platform === "android" ? ANDROID_TABLE : IOS_TABLE;
    const safeDisplayVersion = displayVersion.replace(/'/g, "\\'");
    const safeBuildVersion = buildVersion.replace(/'/g, "\\'");
    const sql = `
...
export async function fetchCrashVersions(platform, token) {
    const table = platform === "android" ? ANDROID_TABLE : IOS_TABLE;
    const sql = `
    SELECT
