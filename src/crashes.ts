...
import {
  desymbolizeWithFlutter,
  extractSymbolsToDir,
  extractSymbolsToTemp,
} from "./desymbolize.js";

async function fatalError(msg: string): Promise<never> {
  p.log.error(msg);
...
function loadCrashNotes(): Record<string, CrashNote> {
  try {
    if (!fs.existsSync(NOTES_FILE)) return {};
    return JSON.parse(fs.readFileSync(NOTES_FILE, "utf8")) as Record<
      string,
      CrashNote
    >;
  } catch {
...
function saveCrashNote(issueId: string, note: CrashNote): void {
...
function groupCrashes(events: CrashEvent[]): CrashGroup[] {
  ...
  for (const ev of events) {
    ...
    if (!existing) {
      ...
      if (ev.timestamp > existing.latestTimestamp) {
        ...
        if (ev.stacktrace) existing.stacktrace = ev.stacktrace;
      }
    }
  }
...
function fatalBadge(isFatal: boolean): string {
  return isFatal ? pc.bgRed(pc.white(" FATAL ")) : pc.bgYellow(pc.black(" non-fatal "));
}

function clipLine(line: string, maxW: number): string {
  return line.length > maxW ? line.slice(0, maxW - 1) + "…" : line;
}

function findSymbolsDir(extractDir: string): string {
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        if (subEntries.some((f) => f.endsWith(".symbols"))) return sub;
      }
      if (entry.name.endsWith(".symbols")) return extractDir;
    }
  } catch {
    // ignore
  }
...
function cacheDirForSymbols(buildId: string): string {
  return path.join(os.homedir(), ".nbuild", "cache", "symbols", buildId);
}

function cacheMarkerPath(
  cacheDir: string,
...
function cacheHasSymbols(
  ...
  if (fs.existsSync(cacheMarkerPath(cacheDir, platform))) {
    ...
    try {
      ...
      if (entries.some((e) => e.isFile() && e.name.endsWith(".symbols"))) return true;
    } catch {
  ...
  try {
    const hasSymbols = (dir: string): boolean => {
      const entries = fs.readdirSync(dir, { withFileTypes: true });
      for (const e of entries) {
        if (e.isFile() && e.name.endsWith(".symbols")) return true;
        if (e.isDirectory() && hasSymbols(path.join(dir, e.name))) return true;
      }
      return false;
    };
...
function isLikelyObfuscatedStacktrace(stacktrace: string): boolean {
  const s = stacktrace.trim();
  if (!s) return false;

  if (s.includes("_kDartIsolateSnapshot")) return true;

  const lower = s.toLowerCase();
  if (lower.includes("package:") || lower.includes(".dart:")) return false;
...
async function browseCrashes(
  groups: CrashGroup[],
  desymbolized: Map<string, string>,
  desymbolizeStatus: Map<string, string>,
  notes: Record<string, CrashNote>,
  version: string,
  platform: string,
  ...
  while (true) {
    ...
    const selected = await p.select({
      message: `v${version} · ${platform} · ${groups.length} unique issue${
        groups.length !== 1 ? "s" : ""
      }`,
      options: listOptions,
    });

    if (p.isCancel(selected) || selected === EXIT) break;
    ...
    if (
      !desymbolized.has(group.issueId) &&
      symbolsDir &&
      group.stacktrace &&
      isObfuscated
    ) {
      ...
      try {
        const result = await desymbolizeWithFlutter(
          group.stacktrace,
          symbolsDir,
        );
    ...
    const maxW = cols - 8;
    const clip = (s: string) => clipLine(s, maxW);
    ...
    const detailLines = [
      ...
      `${pc.cyan("Symbols    ")}  ${
        desymbolizeStatus.get(group.issueId) ?? "pending"
      }`,
      "",
    ...
    if (action === FULL_STACK) {
      p.note(
        stackLines.map((l) => pc.dim(clip(l))).join("\n"),
        "Full Stack Trace",
      );
      continue;
    }
...
export async function runCrashesCommand(
  projectRoot: string,
): Promise<void> {
  // 1. Select platform.
  ...
  const platform = platformChoice as "android" | "ios";
  ...
  try {
    ...
    if (err instanceof FirebaseReauthRequiredError) {
      ...
      try {
        ...
        return await fatalError(
          `Auth failed: ${err2 instanceof Error ? err2.message : String(err2)}`,
        );
      }
    } else {
      return await fatalError(
  ...
  if (!appId) {
    return await fatalError(
      `Could not determine Firebase App ID for ${platform}. ` +
        `Check that ${
          platform === "android"
  ...
  const selected = selectedVersion as { displayVersion: string; buildVersion: string };
  ...
  try {
    crashEvents = await fetchCrashesForVersion(
      platform,
      selected.displayVersion,
      selected.buildVersion,
      token,
    );
  ...
  let buildId: string;
  try {
    const uuid = await fetchBuildIdForVersion(
      platform,
      selected.displayVersion,
      selected.buildVersion,
      token,
  ...
  const flavors: Array<"stage" | "sandbox" | "production"> = [
    "stage",
    "sandbox",
    "production",
  ];
  const symbolTarName =
    platform === "android" ? "androidsymbols.tar.gz" : "iossymbols.tar.gz";
  let gcsSymbolPath = "";
  const desymbolized = new Map<string, string>();
  const desymbolizeStatus = new Map<string, string>();
  let symbolsDir: string | null = null;
  ...
  if (cacheHasSymbols(cacheDir, platform)) {
    ...
    try {
      ...
      symSpinner.stop(
        `Could not check GCS: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
    ...
    if (hasSymbols) {
      ...
      try {
        const tmpTar = path.join(
          os.tmpdir(),
          `nbuild-flutter-sym-${Date.now()}.tar.gz`,
        );
        ...
        symSpinner.stop(
          `Symbol download failed: ${err instanceof Error ? err.message : String(err)}`,
        );
