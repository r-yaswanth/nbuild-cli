...
function isTruthyArg(flag: string): boolean {
...
function findUpward(startDir: string, maxDepth: number): string[] {
  const dirs: string[] = [];
  let dir = startDir;
...
function detectGitOriginFromPackage(): string | null {
...
async function updateViaNpmLatest(): Promise<boolean> {
  const currentVersion = readLocalVersion();

  const latestRes = await exec(
    "npm",
  ...
  if (!skipPrompt) {
    ...
    if (p.isCancel(ok)) return true;
    if (!ok) return true;
  }

  const s = p.spinner();
  s.start(`Updating to ${pc.green(latestVersion)}...`);
  const installRes = await exec(
    "npm",
...
async function updateViaGit(): Promise<boolean> {
  const gitUrlArg =
    getArgValue("--git-url") ?? getArgValue("--git") ?? getArgValue("--repo");
  ...
  if (!detected) {
    p.log.error(
      pc.red(
        "Can't auto-detect git source. Please pass --git-url <url>.",
      ),
    );
    return false;
  }
...
export async function runUpdateCommand(): Promise<void> {
  ...
  try {
    if (s) s.start("Checking npm...");

    const npmCheck = await exec("npm", ["--version"], { cwd: process.cwd() });
    if (npmCheck.exitCode !== 0) {
      throw new Error(
        npmCheck.stderr || npmCheck.stdout || "npm not found or not working",
      );
    }
    ...
    if (s) s.stop(pc.yellow("npm package not found; trying git update..."));
    else p.log.warn(pc.yellow("npm package not found; trying git update..."));

    await updateViaGit();
  } catch (err: AnyError) {
    const msg = err instanceof Error ? err.message : String(err);
    if (s) s.stop(pc.red("❌ Update failed"));
    else p.log.error(pc.red("❌ Update failed"));
    p.log.error(pc.dim(msg));

    // Extra detail if it was exec result shaped
    if (typeof err === "object" && err && "stderr" in err) {
      try {
        const tail = lastErrorLines(err as any);
