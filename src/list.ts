...
interface TableRow {
  ...
  label: string;    // "APK" | "IPA"
  version: string;  // "1.0.0 (4)"
  env: string;      // "stage" | "sandbox" | "production" | "—"
  branch: string;
  commit: string;
  date: string;
}
...
function parseMetaFromNotes(notes: string): { branch: string; commit: string; env: string } {
  const branchMatch = notes.match(/branch:\s*(.+)/i);
  const commitMatch = notes.match(/commit:\s*([a-f0-9]+)/i);
  const envMatch =
    notes.match(/environment:\s*(.+)/i) ??
    notes.match(/build\s+type[:\s]+(.+)/i);

  let env = envMatch ? envMatch[1].trim() : "";
  const envLow = env.toLowerCase();
  if (envLow.includes("stag")) env = "stage";
  else if (envLow.includes("sandbox")) env = "sandbox";
  else if (envLow.includes("prod")) env = "production";
  else env = "—";
...
function toRow(release: FirebaseRelease, platform: PlatformKey): TableRow {
...
function colorEnv(env: string, w: number): string {
...
function makeBorderLine(
  left: string, sep: string, right: string, fill: string, widths: number[],
): string {
...
function renderTable(
  ...
  function dataLine(row: TableRow, highlighted: boolean): string {
  ...
  const n = rows.length;
  lines.push(
    `  ${pc.dim(`${n} build${n !== 1 ? "s" : ""}   ↑↓ navigate  Enter select  Esc quit`)}`,
  );
...
function selectFromTable(rows: TableRow[], showPlatform: boolean): Promise<TableRow | null> {
  return new Promise((resolve) => {
    ...
    function finish(row: TableRow | null) {
...
async function fetchAllReleases(
  ...
  projectRoot: string,
): Promise<{ rows: TableRow[]; token: string }> {
  ...
  allRows.sort(
    (a, b) =>
      new Date(b.release.createTime).getTime() -
...
function wrap(text: string, maxW = WRAP_WIDTH): string[] {
...
interface ParsedNotes {
  body: string[];           // formatted lines for main body
  codeDetails: [string, string][];  // key-value pairs from "Code Details" section
}
...
function renderMarkdownTable(tableLines: string[]): string[] {
  const dataRows = tableLines.filter(
    (l) => !/^\|[\s\-:]+(\|[\s\-:]+)*\|?\s*$/.test(l.trim()), // skip separator rows
  );
  ...
  const colW = Array.from({ length: colCount }, (_, ci) =>
    Math.max(...parsed.map((r) => (r[ci] ?? "").length), 0),
  );
...
function parseNotes(raw: string): ParsedNotes {
  ...
  const codeDetails: [string, string][] = [];
  for (const line of detailRaw.split("\n").slice(1)) {
    const m = line.trim().match(/^([^:]+):\s*(.+)$/);
    if (m) codeDetails.push([m[1].trim(), m[2].trim()]);
  }
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    if (trimmed.startsWith("|")) {
      ...
      pushBlank();
      for (const tl of renderMarkdownTable(tableLines)) body.push(tl);
      continue;
    }
    ...
    if (/^\s*-{1,2}\s+/.test(input[i])) {
      ...
      for (const r of rest) body.push(`    ${r}`);
      continue;
    }
    ...
    if (kv) {
      ...
      for (const r of rest) body.push(`  ${" ".repeat(key.length + 2)}${r}`);
      continue;
    }
    ...
    if (
      trimmed.length < 36 && !trimmed.includes(":") && !trimmed.startsWith("-") &&
...
function renderCodeDetailsCard(details: [string, string][], width: number): string[] {
...
function isMdfriedAvailable(): boolean {
  return spawnSync("which", ["mdfried"], { encoding: "utf-8" }).status === 0;
}

function viewWithMdfried(raw: string): void {
...
async function viewReleaseNotes(row: TableRow): Promise<void> {
  ...
  function draw() {
    ...
    const status =
      `  ${pc.dim("↑↓ / jk  scroll    PgUp/PgDn  page    q  quit")}` +
      `${pc.dim("  ")}${pc.dim(pct.padStart(4))}`;

    const out: string[] = [];
    out.push("\x1b[H\x1b[2J"); // clear & home
    out.push("");
    for (const h of HEADER_LINES) out.push(h);
    ...
    for (let i = 0; i < padRows; i++) out.push("");
  ...
  await new Promise<void>((resolve) => {
    ...
    function onData(data: Buffer) {
      ...
      if (key === "q" || key === "\x1b") { cleanup(); resolve(); return; }

      if (key === "\x1b[A" || key === "k") { scrollOff = Math.max(0, scrollOff - 1); }
      else if (key === "\x1b[B" || key === "j") { scrollOff++; }
      else if (key === "\x1b[5~" || key === "\x1b[I") { scrollOff = Math.max(0, scrollOff - viewH); }    // PgUp
      else if (key === "\x1b[6~" || key === "\x1b[G") { scrollOff += viewH; }                             // PgDn
      else if (key === "g") { scrollOff = 0; }                                 // top
      else if (key === "G") { scrollOff = contentLines.length; }               // bottom
...
async function editReleaseNotes(row: TableRow, token: string): Promise<void> {
...
async function editTesters(
  ...
  let testers: Awaited<ReturnType<typeof listTesters>>;
  ...
  const selectedTesters =
    testers.length > 0
      ? (await searchMultiselect(
        "👤 Select individual testers to add",
        testers.map((t) => ({
          value: t.email,
  ...
  const extraEmails =
    !p.isCancel(extraInput) && extraInput.trim()
      ? extraInput
        .split(",")
        .map((e) => e.trim())
        .filter((e) => e.includes("@"))
...
async function confirmDeleteBuild(
  ...
  p.log.warn(
    `⚠️  You are about to permanently delete ${pc.bold(`${row.label} ${row.version}`)} — ${row.date}`,
  );
...
async function handleBuildActions(
  row: TableRow,
  token: string,
): Promise<"deleted" | "done"> {
  p.log.info(
    `${pc.bold(row.label + " " + row.version)} · ${row.env} · ${row.branch} · ${row.date}`,
  );
  ...
  if (p.isCancel(action)) return "done";
...
export async function runListCommand(projectRoot: string): Promise<void> {
  // 1. Platform selection
  const platformInput = await p.multiselect({
    message: "Select platforms",
    options: [
      { value: "android" as PlatformKey, label: "Android  (APK)" },
      { value: "ios" as PlatformKey, label: "iOS  (IPA)" },
    ],
    initialValues: ["android" as PlatformKey, "ios" as PlatformKey],
    required: true,
  });
  ...
  const platforms = platformInput as PlatformKey[];
