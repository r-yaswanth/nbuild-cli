...
export async function desymbolizeWithFlutter(
  stacktrace: string,
  ...
  try {
    ...
    for (const symbolFile of pathsToTry) {
      const result = await execShell(
        `flutter symbolize --input "${tmpInput}" --debug-info "${symbolFile}"`,
        { cwd: os.tmpdir() },
      );
...
export async function extractSymbolsToTemp(tarGzPath: string): Promise<string> {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nbuild-extract-"));
  await extractSymbolsToDir(tarGzPath, tmpDir);
  return tmpDir;
}

export async function extractSymbolsToDir(
  tarGzPath: string,
  destDir: string,
): Promise<void> {
  ...
  if (result.exitCode !== 0) {
    throw new Error(
      `Failed to extract symbols: ${result.stderr.slice(-500)}`,
    );
  }
}
