...
function cleanupOutputDirs(projectRoot: string): void {
  let removed = 0;
  try {
...
function findBuildRunnerDirs(root: string): string[] {
  const result: string[] = [];
...
interface Step {
  startMsg: string;
  successMsg: string;
  run: () => Promise<import("./exec.js").ExecResult>;
}
...
export async function runBuildPipeline(config: BuildConfig): Promise<void> {
  ...
  for (const step of steps) {
    ...
    if (result.exitCode !== 0) {
      if (s) s.stop(pc.red(`❌ ${step.startMsg} failed`));
      else p.log.error(`❌ ${step.startMsg} failed`);
    ...
    if (s) s.stop(pc.green(`✅ ${step.successMsg}`));
    else p.log.success(step.successMsg);
  }
}
