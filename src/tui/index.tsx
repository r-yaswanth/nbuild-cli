...
export function loadArchives(archivesPath: string): ArchiveEntry[] {
  const entries: ArchiveEntry[] = [];
  const flavors = ["sandbox", "staging", "stage", "production"];

  for (const flavor of flavors) {
    const flavorDir = path.join(archivesPath, flavor);
    if (!fs.existsSync(flavorDir)) continue;

    let buildIds: string[];
    try { buildIds = fs.readdirSync(flavorDir); }
    catch { continue; }

    for (const buildId of buildIds) {
      const buildDir = path.join(flavorDir, buildId);
      try { if (!fs.statSync(buildDir).isDirectory()) continue; }
      catch { continue; }
      ...
      if (fs.existsSync(metadataPath)) {
        try {
          const meta = JSON.parse(fs.readFileSync(metadataPath, "utf-8")) as { version?: string; createdAt?: string };
      ...
      if (fs.existsSync(path.join(buildDir, "androidsymbols.tar.gz"))) platforms.push("android");
      if (fs.existsSync(path.join(buildDir, "iossymbols.tar.gz")))     platforms.push("ios");
...
export function runArchivesTUI(archivesPath: string): void {
  ...
  const { waitUntilExit, unmount } = render(
    <App entries={entries} termRows={termRows} termCols={termCols} />,
    { exitOnCtrlC: true },
  );
