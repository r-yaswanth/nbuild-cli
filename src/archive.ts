...
import { androidDsymsDir, buildArchiveDir, iosDsymsPath, type BuildConfig } from "./config.js";
...
function flutterSplitDebugInfoDir(root: string): string {
  return path.join(root, "output");
}

function hasFlutterSymbols(dir: string): boolean {
  ...
  try {
    const walk = (d: string): boolean => {
      ...
      for (const entry of entries) {
        ...
        if (entry.isFile() && entry.name.endsWith(".symbols")) return true;
...
function copyFlutterSymbols(outputDir: string, destDir: string): number {
  ...
  const walk = (dir: string): void => {
    ...
    for (const entry of entries) {
      ...
      if (!entry.name.endsWith(".symbols")) continue;
...
function parsePubspecLock(content: string): Record<string, LockPackageInfo> {
  const resolved: Record<string, LockPackageInfo> = {};
  ...
  for (const rawLine of content.split("\n")) {
    ...
    if (!currentPackage || !line.startsWith("    ")) continue;
...
async function collectLocalChangesMetadata(
  projectRoot: string,
): Promise<LocalChangesMetadata> {
...
function collectDependenciesMetadata(
  projectRoot: string,
): Record<string, LockPackageInfo> {
  const pubspecPath = path.join(projectRoot, "pubspec.yaml");
  const lockPath = path.join(projectRoot, "pubspec.lock");
  if (!fs.existsSync(pubspecPath) || !fs.existsSync(lockPath)) return {};
  const lock = fs.readFileSync(lockPath, "utf-8");
  return parsePubspecLock(lock);
}

export async function archiveBuild(config: BuildConfig): Promise<void> {
  ...
  const tarResult = await execShell(
    `tar ${excludeArgs} -czf "${sourceDest}" -C "${config.projectRoot}" .`,
    { cwd: config.projectRoot },
  );
